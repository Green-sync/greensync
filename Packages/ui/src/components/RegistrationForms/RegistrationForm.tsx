                                ...
                                FarmFormStaticData.map((input, index) => {
                                    return (
                                        <div key={index}>
                                            ...
                                            {
                                                input.type === 'text' ? <input type={input.type} name={input.name} id={input.id} className=" border-b border-grey-600 text-lime-900 text-sm  block w-full p-2.5 outline outline-0 transition-all placeholder-shown:border-blue-gray-200 focus:border-lime-500 focus:outline-0 disabled:border-0 disabled:bg-blue-gray-50" placeholder={input.placeholder} required={true} /> :
                                                    input.type === 'radio' ? <>

                                                        <div key={index} className="relative h-10 w-72 min-w-[200px] mt-4">
                                                            <select className="peer mt-2 h-full w-full rounded-[7px] border border-blue-gray-200  bg-transparent px-3 py-2.5 font-sans text-sm font-normal text-blue-gray-700 outline outline-0 transition-all placeholder-shown:border placeholder-shown:border-blue-gray-200 placeholder-shown:border-blue-gray-200 empty:!bg-lime-600 focus:border-2 focus:border-lime-500 focus:outline-0 disabled:border-0 disabled:bg-blue-gray-50">
